 ...
 * VERSION : 2.4 - RESTAURATION COMPLÈTE LOGIQUE PROD & PWA
 */
...
const SNIPPET_HEAD = "<!-- SNIPPET : PORTAIL CLIENT V2.4 nuclear prod -->" +
...
const CSS_CONTENT = escapeString(`
    ...
    .pwa-login-btn { display: flex; align-items: center; justify-content: center; gap: 10px; width: 100%; margin-top: 16px; padding: 14px 20px; background: transparent; border: 1px solid rgba(135, 50, 118, 0.5); border-radius: 50px; color: rgba(255,255,255,0.75); font-weight: 700; cursor: pointer; transition: 0.3s; }
    .pwa-login-btn:hover { background: rgba(135, 50, 118, 0.15); border-color: var(--dash-primary); color: #fff; }
    ...
    .cockpit-center span { font-size: 2.5rem; font-weight: 900; color: #fff; }
    ...
    .step-node { position: relative; z-index: 2; width: 50px; height: 50px; background: #000; border: 2px solid var(--dash-border); border-radius: 50%; display: flex; align-items: center; justify-content: center; }
...
<div class="portal-main-wrapper">
    ...
    <div class="portal-container" id="portalDash">
        <header style="margin-bottom: 60px;"><h1 id="txtWelcome" style="font-family:'Outfit'; font-size:3.5rem; font-weight:900; margin:0;">--</h1><p id="txtCodeRef" style="opacity:0.6; font-weight:700;">Dossier #--</p></header>
        <div style="background:rgba(255,255,255,0.03); padding:40px; border-radius:40px; border:1px solid var(--dash-border); display:flex; align-items:center; gap:40px; margin-bottom:40px; flex-wrap:wrap;">
            ...
            <div style="text-align:center;"><div class="cockpit-ring-wrap"><svg class="cockpit-svg" viewBox="0 0 120 120"><defs><linearGradient id="ringGrad" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#873276"/><stop offset="100%" stop-color="#ff4db8"/></linearGradient></defs><circle class="ring-bg" cx="60" cy="60" r="50"/><circle class="ring-progress" id="ringProgress" cx="60" cy="60" r="50"/></svg><div class="cockpit-center"><span id="ringPct">0%</span><small style="opacity:0.5; font-size:0.7rem; font-weight:800; text-transform:uppercase;">Avancement</small></div></div></div>
        </div>
...
const JS_LOGIC = `
    ...
    function renderPortal(data) {
        ...
        document.getElementById('wrapTeam').innerHTML = data.team.map(t => '<div class="team-card"><div style="font-size:2rem;">'+t.icon+'</div><div style="font-weight:800;">'+t.name+'</div><div style="opacity:0.6;font-size:0.8rem;">'+t.role+'</div><div style="font-size:0.7rem;margin-top:5px;">'+t.phone+'</div></div>').join('');
    ...
    async function handlePortalLogin(e) {
        ...
        try {
            ...
            btn.innerText = "Ouvrir mon dossier"; btn.disabled = false;
        }
    }
    ...
    async function installPortalApp() {
        if (!deferredPrompt) {
            const code = document.getElementById('inpPortalCode').value.toUpperCase() || 'EP-1';
            window.location.href = "https://dossier.evanpatruno.ca/mon-dossier.html?code=" + code;
            return;
        }
    ...
    if ('serviceWorker' in navigator) {
        window.addEventListener('load', () => {
            ...
            if (isStandalone) {
                const b = document.getElementById('btnPwaLogin');
...
const ROOT_FILE = "<!DOCTYPE html>\n<html lang='fr'><head><meta charset='UTF-8'><title>Portail Client | Evan Patruno</title><style>body{margin:0;background:#1a0516;}\n" + CSS_CONTENT + "\n</style></head>\n<body>\n" + HTML_BODY + "\n<script>\n" + JS_LOGIC + "\n</script></body></html>";
fs.writeFileSync('c:/Users/evanp/OneDrive/Desktop/PROGRAMME AI/evanpatruno.ca/mon-dossier.html', ROOT_FILE, 'utf8');

console.log('Regeneration Successful with Production Logic.');
